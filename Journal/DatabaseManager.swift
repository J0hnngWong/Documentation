...
class DatabaseManager {
    ...
    var tableName: String = "journal_list"
    
    init() {
        self.openDB()
        self.createTable(tableName: tableName, parameterList: [:])
    }
    ...
    func queryData(id: UUID? = nil) -> Array<JournalModel> {
        ...
        var cSql = "SELECT id,title,detail,image_name,create_date,last_update_date FROM '\(tableName)';".cString(using: .utf8)
        if let idTmp = id {
            cSql = "SELECT id,title,detail,image_name,create_date,last_update_date FROM '\(tableName)' WHERE id='\(idTmp)';".cString(using: .utf8)
        }
